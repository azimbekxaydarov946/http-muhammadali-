        ...
        <div class="container">
            <div class="padding-top padding-bottom">
                <div class="account-area">
                    <h2 class="title" style="margin-bottom: 20px; display: flex;justify-content: center;">To'lov o'zgartirish
                    </h2>
                    <form class="account-form" action="{{ route('home.update', ['id' => $data->id]) }}" method="post">
                        @csrf
                        @method('put')
                        <div class="form-group">
                            ...
                            <select name="user_id" id="user_id" style="background: #001232; ">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{isset($data)? (!empty($data->user_id == $user->id)? 'selected':''):''}}>{{ $user->last_name }}{{ $user->first_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" placeholder="Ismgizni kiriting" id="amount" required name="amount" value="{{$data->amount}}">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="debit_cost" id="debit_cost" style="background: #001232; ">
                                <option value="1" {{(!empty($data->debit_cost == 1)? 'selected':'')}}>Kirim</option>
                                <option value="0" {{(!empty($data->debit_cost == 0)? 'selected':'')}}>Chiqim</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="pay_type">Foydalanuvchi<span>*</span></label>
                            <select name="pay_type" id="pay_type" style="background: #001232; ">
                                <option value="1" {{(!empty($data->pay_type == 1)? 'selected':'')}}>Naqd</option>
                                <option value="0" {{(!empty($data->pay_type == 0)? 'selected':'')}}>Plastik</option>
                            </select>
                        </div>
                        <div class="form-group">
